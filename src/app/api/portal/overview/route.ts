...
export async function GET(req: NextRequest) {
  try {
    ...
    const [attendance, results, invoices, payments, transport, notices, routine, homework, messages, leaves] = await Promise.all([
      ...
      prisma.invoice.findMany({ where: { studentId }, orderBy: { createdAt: "desc" }, take: 30 }),
      prisma.payment.findMany({ where: { invoice: { studentId } }, include: { invoice: true }, orderBy: { createdAt: "desc" }, take: 30 }),
      prisma.studentTransport.findMany({ where: { studentId, status: "ACTIVE" }, include: { route: true, stop: true } }),
      prisma.notice.findMany({ orderBy: [{ pinned: "desc" }, { createdAt: "desc" }], take: 12 }),
      student.classId ? prisma.routineSlot.findMany({ where: { classId: student.classId, OR: [{ sectionId: student.sectionId }, { sectionId: null }] }, include: { subject: true, teacher: true } }) : [],
      student.classId ? prisma.homework.findMany({ where: { classId: student.classId, OR: [{ sectionId: student.sectionId }, { sectionId: null }] }, include: { subject: true, teacher: true }, orderBy: { dueDate: "desc" }, take: 30 }) : [],
      prisma.parentMessage.findMany({ where: { studentId }, include: { teacher: true }, orderBy: { createdAt: "asc" }, take: 100 }),
      prisma.leaveRequest.findMany({ where: { studentId }, orderBy: { createdAt: "desc" }, take: 30 }),
    ]);
