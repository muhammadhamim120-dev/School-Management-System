...
export async function GET(req: NextRequest) {
  try {
    ...
    const where = search ? { OR: [{ title: { contains: search, mode: "insensitive" as const } }, { content: { contains: search, mode: "insensitive" as const } }] } : {};
...
export async function POST(req: NextRequest) {
  try {
    ...
    return created(await prisma.notice.create({ data }));
  } catch (e) { return handleError(e); }
}
