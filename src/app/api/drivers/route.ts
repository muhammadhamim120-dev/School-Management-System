...
export const GET = withTenantContext(async (req: NextRequest) => {
  try {
    ...
    const items = await prisma.driver.findMany({ where, orderBy: { name: "asc" } });
...
export const POST = withTenantContext(async (req: NextRequest) => {
  try {
    const data = driverSchema.parse(await req.json());
    return created(await prisma.driver.create({ data: { ...data, schoolId: getRequiredTenantId() } }));
  } catch (e) { return handleError(e); }
});
