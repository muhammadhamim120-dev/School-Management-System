...
export const GET = withTenantContext(async (req: NextRequest) => {
  try {
    ...
    const where = tenantWhere(search ? { OR: [
...
export const POST = withTenantContext(async (req: NextRequest) => {
  try {
    ...
    const route = await prisma.transportRoute.create({ data: {
      ...
      stops: { create: (data.stops ?? []).map((s, i) => ({ name: s.name, sequence: s.sequence ?? i, pickupTime: s.pickupTime || null, schoolId: getRequiredTenantId() })) },
