...
import { smsTemplateSchema } from "@/lib/validations";
...
export const GET = withTenantContext(async (req: NextRequest) => {
  try {
    ...
    const items = await prisma.smsTemplate.findMany({ where, orderBy: { name: "asc" } });
...
export const POST = withTenantContext(async (req: NextRequest) => {
  try {
    const data = smsTemplateSchema.parse(await req.json());
    const schoolId = getRequiredTenantId();
    return created(await prisma.smsTemplate.create({ data: { ...data, schoolId } }));
  } catch (e) { return handleError(e); }
});
