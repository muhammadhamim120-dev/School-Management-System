...
import { Shield, X, PanelLeftClose, PanelLeft } from "lucide-react";
import { superAdminNavGroups } from "@/components/super-admin/nav";
...
const STORAGE_KEY = "sms-super-admin-sidebar-collapsed";

export function SuperAdminSidebar({ open, onClose, user }: Props) {
    ...
    <>
      ...
      >
        ...
        <div className={cn(
          ...
          <Link href="/super-admin" className={cn("flex items-center gap-2.5", isCollapsed && "justify-center")}>
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-red-500 to-red-700 text-white shadow-sm">
              <Shield className="h-[18px] w-[18px]" />
            </span>
            {!isCollapsed && (
              <div className="leading-tight animate-fade-in">
                <div className="text-sm font-semibold tracking-tight">Super Admin</div>
                <div className="text-[11px] text-muted-foreground">Platform Control</div>
              </div>
            )}
          </Link>
          {!isCollapsed && (
        ...
        <nav className="flex-1 overflow-y-auto px-2.5 py-3">
          {superAdminNavGroups.map((group) => (
            <div key={group.label} className={cn("mb-4", isCollapsed && "mb-3")}>
              {!isCollapsed && (
                <div className="mb-1 px-2.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground/60">
                  {group.label}
                </div>
              )}
              {isCollapsed && <div className="mx-auto mb-1 h-px w-5 bg-border" />}
          ...
          <div className={cn(
            ...
            {!isCollapsed && (
              <div className="min-w-0 flex-1 leading-tight animate-fade-in">
                <div className="truncate text-sm font-medium">{user?.name || "Super Admin"}</div>
                <div className="truncate text-[11px] text-muted-foreground">{user?.email || "admin@platform.com"}</div>
              </div>
            )}
          </div>
        </div>
